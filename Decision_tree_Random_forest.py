...
def random_forest(X_train, y_train, n_trees = 10, max_depth = 2, min_samples_leaf = 5, min_impurity_decrease = 0):
    trees = []
    for i in range(n_trees):
        ...
        try:
            tree = build_tree(X_bootstrap, y_bootstrap, depth = 0, max_depth = max_depth, min_samples_leaf = min_samples_leaf, min_impurity_decrease = min_impurity_decrease)
            trees.append(tree)
        except ValueError as e:
            print(f"Error while building tree {i+1}: {e}")
    return trees
...
Forest = random_forest(X, y, n_trees=10, max_depth=2, min_samples_leaf=5, min_impurity_decrease=0)
print("Forest = ", Forest)
